...
function QuestionList() {
  ...
  const fetchQuestionList = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/QuizApp/quiz/${id}/questions`
      );
      if (!response.ok) {
  ...
  const fetchAnswer = async (questionId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/QuizApp/questions/${questionId}/answer`
      );
      if (!response.ok) {
        throw new Error(
          "Error in retrieving question answer" + response.statusText
        );
      }
  ...
  const postAnswer = async (questionId, userAnswer) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/QuizApp/questions/${questionId}/answers`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <ThemeProvider theme={theme}>
      <Container maxWidth="lg" style={{ marginTop: "50px" }}>
        ...
        <h1
          style={{ fontSize: "44px", fontWeight: "bold", marginBottom: "16px" }}
        >
        ...
        <Snackbar
          ...
          message={
            correct
              ...
              : `That is not correct the correct answer is '${correctAnswer}' !`
          }
        />
      </Container>
    </ThemeProvider>
